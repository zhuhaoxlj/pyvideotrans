...
class WhisperTimestampChecker(QMainWindow):
    ...
    def __init__(self):
        ...
        self.srt_file = None  # 添加字幕文件支持
        self.cache_file = None
        self.words_data = []
        self.current_word_index = -1
        
        self.setWindowTitle("🔍 Whisper 词级时间戳检测工具")
    ...
    def init_ui(self):
        ...
        # 标题
        title_label = QLabel("🔍 Whisper 词级时间戳精确度检测器")
        ...
        info_label = QLabel(
            "📝 使用说明：\n"
            "1. 选择视频文件（已处理过的视频）\n"
            "2. 工具会自动查找对应的 Whisper 缓存\n"
            "3. 点击任意单词，视频会跳转到该单词的时间戳位置\n"
            "4. 观察视频中实际说这个词的时间是否和时间戳一致\n"
            "5. 如果不一致，说明 Whisper 时间戳有偏差"
        )
        if self.is_dark_theme:
        ...
        video_layout.addWidget(self.video_file_label, 1)
        btn_layout.addLayout(video_layout)
        
        # 字幕文件选择（可选）
        srt_layout = QHBoxLayout()
        self.select_srt_btn = QPushButton("📄 选择字幕文件（可选）")
        self.select_srt_btn.setMinimumHeight(50)
        ...
        """)
        self.select_srt_btn.clicked.connect(self.select_srt)
        srt_layout.addWidget(self.select_srt_btn)
        
        self.srt_file_label = QLabel("未选择字幕文件（如果使用了原字幕重新分割，请选择原字幕文件）")
        self.srt_file_label.setStyleSheet(f"QLabel {{ color: {label_color}; padding: 10px; }}")
        srt_layout.addWidget(self.srt_file_label, 1)
        btn_layout.addLayout(srt_layout)
    ...
    def select_srt(self):
        ...
        self.srt_file_label.setText(f"✅ {Path(file_path).name}")
    ...
    def try_load_cache(self):
        ...
        if self.find_cache():
            ...
            if self.load_cache():
                ...
                cache_type = "视频+字幕" if self.srt_file else "仅视频"
                self.status_label.setText(f"✅ 成功加载！（{cache_type} 缓存）点击任意单词开始检测")
                if self.is_dark_theme:
                    self.status_label.setStyleSheet("""
                        QLabel {
                            background-color: #1b5e20;
            ...
            if self.srt_file:
                hint = (
                    "未找到该视频+字幕的 Whisper 缓存文件。\n\n"
                    "可能原因：\n"
                    "1. 字幕文件选错了（应该选择原始字幕，不是生成的字幕）\n"
                    "2. 还没有用这个视频+字幕组合处理过\n\n"
                    "提示：如果不确定是否使用了字幕，可以不选择字幕文件。"
                )
            else:
                hint = (
                    "未找到该视频的 Whisper 缓存文件。\n\n"
                    "可能原因：\n"
                    "1. 还没有用 LLM 智能字幕生成工具处理该视频\n"
                    "2. 如果使用了原字幕重新分割，请点击【选择字幕文件】选择原字幕"
                )
    ...
    def find_cache(self):
        ...
        # 如果有字幕文件，也计算字幕哈希
        cache_key = video_hash
        if self.srt_file:
            srt_hash = self.get_file_hash(self.srt_file)
            if not srt_hash:
                return False
            cache_key = f"{video_hash}_{srt_hash}"
